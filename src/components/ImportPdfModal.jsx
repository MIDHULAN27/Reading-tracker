...
export default function ImportPdfModal({ isOpen, onClose }) {
  ...
  const [coverColor, setCoverColor] = useState(COZY_GRADIENTS[0].class);
  ...
  const handleFileProcess = async (selectedFile) => {
    ...
    const selectedGrad = COZY_GRADIENTS[hash % COZY_GRADIENTS.length].class;
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      >
        {/* Header */}
        <div className="flex justify-between items-center border-b border-cozy-cream-300/40 dark:border-cozy-night-100/10 pb-4">
          <div>
            <h2 className="font-serif text-xl sm:text-2xl font-bold text-cozy-night-300 dark:text-white flex items-center gap-2">
              <Book className="w-6 h-6 text-cozy-amber" />
          ...
          <button 
            onClick={onClose}
            className="p-2 rounded-xl bg-black/5 dark:bg-white/5 border border-white/5 hover:bg-black/10 dark:hover:bg-white/10 text-cozy-night-100/50 dark:text-cozy-cream-200/40"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        {success ? (
          ...
          >
            ...
            <h3 className="font-serif text-xl font-bold text-cozy-night-300 dark:text-white">Import Complete!</h3>
            <p className="text-xs text-cozy-night-100/50 dark:text-cozy-cream-200/40 max-w-sm">
          ...
          <div className="space-y-6">
            ...
            {!file ? (
              <div 
                ...
                className={`relative group cursor-pointer p-8 sm:p-12 border-2 border-dashed rounded-2xl flex flex-col items-center justify-center text-center space-y-3 transition-all duration-300 ${
                  dragActive 
                    ? 'border-cozy-amber bg-cozy-amber/5' 
                    : 'border-cozy-cream-300 hover:border-cozy-amber bg-black/5 dark:bg-white/5'
                }`}
              >
                <input 
                  type="file" 
                ...
                <div className="p-4 rounded-full bg-white dark:bg-white/5 border border-white/10 shadow-sm transform group-hover:scale-110 duration-300">
                  <UploadCloud className="w-8 h-8 text-cozy-amber" />
                </div>
                
                <div className="space-y-1">
                  <p className="font-serif font-bold text-sm sm:text-base text-cozy-night-300 dark:text-white">
                    Drag & Drop your legal PDF book here
                  </p>
                  <p className="text-xs text-cozy-night-100/40 dark:text-cozy-cream-200/35">
              ...
              <div className="p-4 rounded-xl bg-black/10 dark:bg-white/5 border border-white/5 flex items-center justify-between gap-4">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-cozy-night-100/40 dark:text-cozy-cream-200/30">
            ...
            {/* Parsing State */}
            {parsing && (
              <div className="flex items-center justify-center gap-2 text-xs font-serif text-cozy-amber animate-pulse">
            ...
            {file && !parsing && (
              <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-2">
                ...
                <div className="flex flex-col items-center justify-center space-y-3">
                  <span className="text-[10px] font-bold text-cozy-night-100/40 dark:text-cozy-cream-200/35 uppercase tracking-widest block text-center w-full">
                    Cover Preview
                  </span>
                ...
                <div className="md:col-span-2 space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <label className="text-[10px] uppercase font-bold tracking-widest text-cozy-night-100/40 dark:text-cozy-cream-200/35 pl-0.5">
                        Book Title *
                      </label>
                      <input 
                        type="text" 
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div className="space-y-1">
                      ...
                      >
                        {GENRES.map(g => (
                          <option key={g} value={g} className="bg-cozy-cream-50 dark:bg-cozy-night-200">{g}</option>
                        ))}
                      </select>
                    </div>
                    <div className="space-y-1">
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] uppercase font-bold tracking-widest text-cozy-night-100/40 dark:text-cozy-cream-200/35 pl-0.5 block">
                      Choose Sanctuary Gradient Style
                    </label>
                    <div className="flex flex-wrap gap-2.5">
                      {COZY_GRADIENTS.map((grad) => (
                        <button
                          key={grad.name}
                          type="button"
                          ...
                          className={`w-7 h-7 rounded-full bg-gradient-to-tr ${grad.class} border-2 transition-all ${
                            coverColor === grad.class 
                              ? 'border-cozy-amber scale-110 ring-2 ring-cozy-amber/25' 
                  ...
                  <div className="flex gap-3 justify-end pt-3 border-t border-cozy-cream-300/20 dark:border-white/5">
                    <button 
                      type="submit"
                      disabled={importing}
                      className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-cozy-amber to-cozy-amber-dark hover:brightness-110 text-white font-bold text-xs shadow-md shadow-cozy-amber/10 active:scale-95 transition-all disabled:opacity-50"
                    >
