...
export const dbService = {
  ...
  verifyConnection: async () => {
    ...
    try {
      ...
      // Lightweight probe query to verify Postgres availability and RLS tables access
  ...
  auth: {
    ...
    guestLogin: async () => {
      try {
        ...
        const guestEmail = `guest-${Math.random().toString(36).substr(2, 9)}@cozyreads.com`;
  ...
  books: {
    ...
    addBook: async (bookData) => {
      ...
      const { data: matchedBooks } = await catalogQuery;
      ...
      if (existingLibrary) {
        return dbService.books._mapJoinedBook(existingLibrary, catalogBook);
      }

      // 3. Add to user library shelf
    ...
    updateBook: async (bookId, updates) => {
      ...
      if (updates.progress !== undefined || updates.pages !== undefined) {
        ...
        if (!totalPages) {
          const { data: b } = await supabase.from('books').select('total_pages').eq('id', bookId).single();
  ...
  logs: {
    ...
    addLog: async (logData) => {
      ...
      if (lib) {
        const { data: b } = await supabase.from('books').select('total_pages').eq('id', logData.book_id).single();
  ...
  progress: {
    ...
    saveProgress: async (bookId, currentLocation, progressPercentage) => {
      ...
      if (fetchError) {
        console.error('Failed to query reading progress:', fetchError);
      }
