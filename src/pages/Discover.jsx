...
export default function Discover() {
  ...
  useEffect(() => {
    ...
    // Fetch lightweight suggestions from books API
  ...
  }, [query, searchBooksDebounced, resetBookSearch]);
  ...
  useEffect(() => {
    if (bookLoading || bookLoadingMore || !bookHasMore || bookResults.length === 0) return;
  ...
  const currentHasMore = bookHasMore;
  const currentResults = filteredResults;
  const currentSearchSource = bookSearchSource;
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm md:text-base text-booklyn-night-100/60 dark:text-booklyn-cream-200/50 max-w-2xl leading-relaxed">
            Search millions of books from global registries. Direct synchronization maps your collection in real-time.
          </p>
        </div>
      </div>
              ...
              >
                ...
                {suggestions.length === 0 ? (
                  <div className="py-4 text-center text-xs text-booklyn-night-100/40 dark:text-booklyn-cream-200/45 flex items-center justify-center gap-2">
                    ...
                    <span>Searching book registry indices...</span>
                  </div>
                ) : (
      ...
      <div className="relative">
                  ...
                  <div key={key} className="space-y-4">
                    ...
                    <div className="flex justify-between items-center select-none">
                      <button 
                        ...
                        className="group/title font-serif text-xl md:text-2xl font-bold tracking-tight text-booklyn-night-300 dark:text-white text-left hover:text-booklyn-amber transition-colors flex items-center gap-2"
                      >
                        <span className={`w-2.5 h-2.5 rounded-full ${getDotColorClass(key)}`} />
                        <span>{category.title}</span>
                        <ChevronRight className="w-4 h-4 opacity-0 -translate-x-2 group-hover/title:opacity-100 group-hover/title:translate-x-0 transition-all text-booklyn-amber" />
                      </button>
                    ...
                    {/* Sliding Netflix Shelf Content */}
                    {category.loading ? (
                      <div className="flex gap-4 overflow-hidden py-2">
                        {Array.from({ length: 5 }).map((_, idx) => (
                          <div 
                            key={idx} 
                            className="w-[140px] sm:w-[160px] md:w-[180px] aspect-[2/3] bg-booklyn-cream-300 dark:bg-booklyn-night-100 rounded-2xl animate-pulse flex-shrink-0"
                          />
                        ))}
                      </div>
                    ) : (
        ...
        {query.trim() !== '' && (
          <div className="space-y-6">
            {/* Filter Bar with Genre pills & Author Select dropdown */}
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  {filteredResults.length !== bookResults.length && (
            ...
            {!bookLoading && filteredResults.length > 0 && (
