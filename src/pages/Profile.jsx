  ...
  const badgesList = [
    {
      id: 'streak_warmth',
      title: '7-Day Warmth',
      description: 'Logged reading logs on 7 consecutive days to build a booklyn, strong habit.',
      icon: Flame,
    ...
    },
    {
      id: 'night_owl',
      title: 'Night Owl',
      description: 'Logged a reading logs session in the booklyn quiet hours between 9 PM and 4 AM.',
      icon: Moon,
    ...
    <div className="relative min-h-screen pb-16">
      ...
      <div className="p-4 sm:p-6 max-w-6xl mx-auto space-y-8 relative z-10 font-sans">
        ...
        <div className="glass-panel border-white/20 dark:border-white/5 rounded-3xl p-6 sm:p-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 relative overflow-hidden">
          {/* Glowing background */}
          <div className="absolute top-0 right-0 w-48 h-48 bg-gradient-to-tr from-booklyn-amber/5 to-booklyn-lavender/5 rounded-full blur-3xl pointer-events-none" />
          
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-tr from-booklyn-amber to-booklyn-amber-dark text-white flex items-center justify-center font-serif text-2xl font-bold shadow-md shadow-booklyn-amber/15">
              {(user?.email || 'CR').charAt(0).toUpperCase()}
            </div>
            <div className="space-y-1">
              <span className="text-[10px] font-bold uppercase tracking-widest text-booklyn-amber dark:text-booklyn-amber-light flex items-center gap-1">
                <Sparkles className="w-3.5 h-3.5" /> Persistent Sanctuary Profile
              </span>
              <h2 className="font-serif font-bold text-2xl sm:text-3xl text-booklyn-night-300 dark:text-white leading-tight">
                {user?.user_metadata?.full_name || user?.email || 'Verified Booklyn'}
              </h2>
              <p className="text-xs text-booklyn-night-100/50 dark:text-booklyn-cream-200/40 font-mono">
                {user?.email || 'local.reader@sandbox.booklyn'}
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
          ...
          <div className="glass-panel border-white/20 dark:border-white/5 rounded-3xl p-5 space-y-2 relative overflow-hidden group">
            ...
            <Flame className="w-6 h-6 text-booklyn-amber" />
            <div className="space-y-0.5">
              <span className="text-[10px] font-bold text-booklyn-night-100/40 dark:text-booklyn-cream-200/35 uppercase tracking-widest block">Active Streak</span>
              <span className="font-sans font-bold text-2xl text-booklyn-night-300 dark:text-white">{streak} days</span>
            </div>
          </div>
          ...
          <div className="glass-panel border-white/20 dark:border-white/5 rounded-3xl p-5 space-y-2 relative overflow-hidden group">
            ...
            <div className="space-y-0.5">
              <span className="text-[10px] font-bold text-booklyn-night-100/40 dark:text-booklyn-cream-200/35 uppercase tracking-widest block">Minutes Read</span>
              <span className="font-sans font-bold text-2xl text-booklyn-night-300 dark:text-white">{totalMinutes} min</span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 glass-panel border-white/20 dark:border-white/5 rounded-3xl p-6 sm:p-8 space-y-6">
            <div className="border-b border-booklyn-cream-300/40 dark:border-booklyn-night-100/10 pb-4">
                ...
                return (
                  <button
                    ...
                    className={`p-4 rounded-2xl border text-left flex gap-4 transition-all duration-300 select-none relative overflow-hidden ${
                      badge.unlocked 
                        ? 'bg-black/5 dark:bg-white/5 border-booklyn-amber/35 shadow-sm cursor-pointer'
                        : 'bg-black/5 dark:bg-white/5 opacity-40 border-white/5 hover:opacity-60 cursor-pointer'
                    }`}
                  >
                    {/* Badge light spot */}
                    {badge.unlocked && (
                      <div className="absolute -bottom-6 -right-6 w-16 h-16 bg-booklyn-amber/5 rounded-full blur-xl pointer-events-none" />
                    )}

                    <div className={`p-3 rounded-xl flex-shrink-0 border h-fit ${
                      badge.unlocked ? badge.color : 'text-booklyn-night-100/30 dark:text-booklyn-cream-200/20 border-white/10'
                    }`}>
                    ...
                    <div className="space-y-1 min-w-0">
                      <div className="flex justify-between items-center gap-2">
                        <h4 className="font-bold text-xs text-booklyn-night-300 dark:text-white truncate">
                          {badge.title}
                        </h4>
                        {badge.unlocked ? (
                          ...
                          <span className="px-1.5 py-0.5 rounded bg-white/10 text-booklyn-night-100/40 dark:text-booklyn-cream-200/30 text-[8px] font-bold tracking-wider uppercase leading-none">
                            Locked
                          </span>
                        )}
                      </div>
                      <p className="text-[10px] text-booklyn-night-100/50 dark:text-booklyn-cream-200/40 line-clamp-2 leading-relaxed">
                        {badge.description}
                      </p>
                      <span className="text-[9px] font-bold uppercase tracking-wider text-booklyn-amber block mt-1.5">
            ...
            <AnimatePresence>
              {activeBadge && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: 10 }}
                  className="p-4 rounded-2xl bg-booklyn-amber/10 border border-booklyn-amber/20 text-booklyn-night-300 dark:text-white space-y-1 text-xs"
                >
                  <h4 className="font-serif font-bold text-sm flex items-center gap-1.5 text-booklyn-amber">
          ...
          <div className="glass-panel border-white/20 dark:border-white/5 rounded-3xl p-6 sm:p-8 space-y-6">
            ...
            <div className="border-b border-booklyn-cream-300/40 dark:border-booklyn-night-100/10 pb-4">
              <h3 className="font-serif font-bold text-lg text-booklyn-night-300 dark:text-white flex items-center gap-2">
                <Network className="w-5 h-5 text-booklyn-amber" />
                <span>Sync Engine details</span>
              </h3>
              <p className="text-[11px] text-booklyn-night-100/50 dark:text-booklyn-cream-200/40 mt-1">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-black/5 dark:bg-white/5 p-4 rounded-2xl border border-white/5 space-y-3">
                <div className="flex justify-between items-center text-xs font-medium">
                  <span className="text-booklyn-night-100/50 dark:text-booklyn-cream-200/40">Database Mode</span>
                ...
                <div className="flex justify-between items-center text-xs font-medium border-t border-white/5 pt-3">
                  <span className="text-booklyn-night-100/50 dark:text-booklyn-cream-200/40">Network connection</span>
                ...
                <div className="flex justify-between items-center text-xs font-medium border-t border-white/5 pt-3">
                  <span className="text-booklyn-night-100/50 dark:text-booklyn-cream-200/40">Queue status</span>
              ...
              <div className="space-y-2">
                <h4 className="text-[10px] uppercase font-bold tracking-widest text-booklyn-night-100/40 dark:text-booklyn-cream-200/35 pl-0.5">
                  Offline simulation lab
                </h4>
                <p className="text-[11px] text-booklyn-night-100/50 dark:text-booklyn-cream-200/40 pl-0.5 leading-relaxed">
                ...
                <button
                  ...
                  className={`w-full py-2.5 rounded-xl font-bold text-xs uppercase tracking-wider transition-all border ${
                    syncState.isSimulated
                      ...
                      : 'bg-white/20 dark:bg-white/5 border-white/10 hover:bg-white/30 text-booklyn-night-300 dark:text-white'
                  }`}
                >
              ...
              {syncState.queueLength > 0 && syncState.online && (
                <button
                  ...
                  className="w-full py-2.5 rounded-xl bg-gradient-to-r from-booklyn-amber to-booklyn-amber-dark text-white font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-1.5 animate-pulse-subtle"
                >
